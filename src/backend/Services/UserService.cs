        ...
        public async Task<UserResponseDto?> SyncFirebaseUserAsync(UserSyncRequestDto syncRequest, bool createNewUser = false)
        {
            ...
            try
            {
                ...
                if (user == null)
                {
                    ...
                    var defaultRole = await _roleRepository.GetRoleByNameAsync(DefaultUserRole);
                    if (defaultRole != null)
                    {
                        if (!user.UserRoles.Any(ur => ur.RoleId == defaultRole.Id))
                        {
                             user.UserRoles.Add(new UserRole { UserId = user.Id, RoleId = defaultRole.Id });
                             _logger.LogInformation("Assigned default role '{DefaultRole}' to new user ID {UserId}", DefaultUserRole, user.Id);
                             await _context.SaveChangesAsync();
                        }
                    }
                    else
                    {
                        _logger.LogWarning("Default role '{DefaultRole}' not found. New user ID {UserId} created without a role.", DefaultUserRole, user.Id);
                    }
